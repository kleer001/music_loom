...
- `sound_feel/` — reproducing an ambient field recording (mall, school; footsteps, shouts, laughter) and generating it endlessly, as original DSP tools emitting seamless stereo loop files — a tribute in spirit to myNoise. Five axes: texture-bed statistics (McDermott–Simoncelli), event extraction and Poisson recirculation, seamless looping (circular-convolution torus), stereo and the moving-mic non-stationarity, and the myNoise reference. `sound_feel/SURVEY.md` is the way in; `sound_feel/BLOCKED.md` is the capture log — the digest was first written search-attested under an egress block, then its load-bearing figures were verified against their primaries (correcting several), with the paywalled, in-bench, and candidate-original items marked as such.
